...
export default function ConseilsAndFAQ() {
  ...
  const themes = [
    ...
    { id: 'tariffs', label: 'Tarifs & Budgets', icon: '💰' },
    { id: 'delays', label: 'Délais & Planning', icon: '⏱️' },
    { id: 'process', label: 'Processus & Étapes', icon: '📋' },
    { id: 'sectors', label: 'Secteurs d\'Intervention', icon: '🗺️' }
  ];
  ...
  const articles: Article[] = [
    ...
    },
    {
      ...
      excerpt: "La perte d'un proche est une épreuve difficile. Découvrez comment gérer le débarras des biens mobiliers de manière respectouveau, organisé et conforme aux exigences notariales.",
      readTime: "4 min",
  ...
  const faqItems: FAQItem[] = [
    ...
    },
    {
      ...
      category: "⏱️ Délais & Planning",
      theme: "delays"
    },
    {
      question: "Quels sont vos secteurs et communes d'intervention autour de Bordeaux et en Gironde ?",
      answer: "Nous intervenons rapidement (sous 24h à 48h) dans toutes les villes dans un rayon de 50km autour de Bordeaux et plus généralement dans l'ensemble du département de la Gironde (33). Les principales communes desservies comprennent : Bordeaux (33000), Mérignac (33700), Pessac (33600), Talence (33400), Villenave-d'Ornon (33140), Saint-Médard-en-Jalles (33160), Bègles (33130), Cenon (33150), Gradignan (33170), Lormont (33310), Eysines (33320), Cestas (33610), Floirac (33270), Blanquefort (33290), Bruges (33520), Ambarès-et-Lagrave (33440), Le Bouscat (33110), Léognan (33850), Saint-André-de-Cubzac (33240), Libourne (33500), Gujan-Mestras (33470), Arcachon (33120), Andernos-les-Bains (33510), Le Haillan (33185), Parempuyre (33290), Martignas-sur-Jalle (33127), Taillan-Médoc (33320), Saint-Loubès (33450), Artigues-près-Bordeaux (33370), Carbon-Blanc (33560), Sainte-Eulalie (33560), Saint-Jean-d'Illac (33127), Le Pian-Médoc (33290)... Nous nous déplaçons gratuitement pour évaluer le volume à vider et vous proposer un devis personnalisé.",
  ...
  const filteredFaqItems = faqItems.filter(item => selectedTheme === 'all' || item.theme === selectedTheme);
    ...
    <div className="py-20 bg-slate-50/40 border-t border-slate-100 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div id="conseils-debarras" className="mb-24">
          <div className="text-center mb-12">
            <span className="inline-block px-3.5 py-1.5 bg-emerald-50 border border-emerald-100/60 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300 dark:border-emerald-900/50 text-[10px] md:text-xs font-bold rounded-full uppercase tracking-wider mb-2 font-sans">
              Conseils & SEO
            </span>
            <h2 className="text-3xl md:text-5xl font-black font-display text-slate-900 dark:text-slate-100 tracking-tight leading-tight">
              Conseils Débarras & Revalorisation
            </h2>
            <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm max-w-xl mx-auto mt-2 font-semibold font-sans">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {articles.map((article, idx) => (
              <div 
                key={idx}
                className="bg-white dark:bg-slate-900/60 rounded-3xl border border-slate-150/80 dark:border-slate-800 p-6 md:p-8 flex flex-col justify-between shadow-xs hover:shadow-md transition-all duration-300 group hover:-translate-y-1"
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase tracking-wider bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 px-3 py-1 rounded-full border border-emerald-100/30 dark:border-emerald-900/30">
                      {article.category}
                    </span>
                    <span className="text-slate-400 dark:text-slate-400 text-[10px] font-bold flex items-center gap-1">
                  ...
                  <h3 className="text-base sm:text-lg font-black text-slate-950 dark:text-slate-100 font-sans tracking-tight leading-tight group-hover:text-emerald-500 transition-colors">
                    {article.title}
                  </h3>
                  
                  <p className="text-slate-500 dark:text-slate-450 text-xs leading-relaxed font-semibold">
                    {article.excerpt}
                  </p>
                </div>
                ...
                <div className="pt-6 mt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <button 
                    ...
                    className="text-xs font-black text-emerald-700 dark:text-emerald-400 group-hover:text-emerald-800 flex items-center gap-1.5 uppercase tracking-wider cursor-pointer"
                  >
        ...
        <div id="faq" className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-500 dark:text-slate-450 text-xs sm:text-sm max-w-xl mx-auto mt-2 font-semibold font-sans">
          ...
          {/* Theme Filters (Elegantly grouped tab system) */}
          <div className="flex flex-wrap justify-center gap-2 mb-8 bg-slate-100/60 dark:bg-slate-800/40 p-2 rounded-2xl border border-slate-200/50 dark:border-slate-800">
            {themes.map((theme) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2.5 rounded-xl text-xs font-extrabold uppercase tracking-wider transition-all duration-200 flex items-center gap-2 cursor-pointer ${
                    isSelected
                      ? 'bg-[#800020] text-white shadow-md scale-[1.02]'
                      : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 border border-slate-200/60 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-800 dark:hover:text-white'
                  }`}
                >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 px-1">
            <span className="text-[11px] font-bold text-slate-400">
              💡 Thème actif : <span className="text-slate-600 font-extrabold uppercase">{themes.find(t => t.id === selectedTheme)?.label}</span>. Cliquez pour déplier :
            </span>
            <div className="flex items-center gap-2 self-end sm:self-auto">
              <button
                onClick={() => {
                  const allOpen: Record<string, boolean> = {};
                  filteredFaqItems.forEach((item, i) => { allOpen[`${selectedTheme}-${i}`] = true; });
          ...
          <div className="space-y-4">
            {filteredFaqItems.length === 0 ? (
              <div className="text-center py-10 text-slate-400 dark:text-slate-500 text-xs font-semibold">
                Aucune question pour ce thème actuellement.
              </div>
            ) : (
              filteredFaqItems.map((item, idx) => {
                const key = `${selectedTheme}-${idx}`;
                const isOpen = !!openStates[key];
                return (
                  <div 
                    key={key}
                    className="bg-white dark:bg-slate-900/60 rounded-2xl border border-slate-150 dark:border-slate-800 overflow-hidden shadow-xs hover:border-slate-300 dark:hover:border-slate-750 transition-colors"
                  >
                    <button
                      onClick={() => {
                        setOpenStates(prev => ({
                      ...
                      className="w-full text-left p-5 flex items-center justify-between gap-4 font-sans font-bold text-slate-900 dark:text-slate-100 text-sm md:text-base hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition cursor-pointer"
                    >
                      <span className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 w-full pr-1">
                        ...
                        {item.category && (
                          <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest bg-slate-50 dark:bg-slate-800 text-emerald-800 dark:text-emerald-300 border border-emerald-100/60 dark:border-emerald-800/60 px-2.5 py-0.5 rounded-md shrink-0 w-fit font-sans">
                            {item.category}
                          </span>
                        )}
                      </span>
                      <ChevronDown className={`w-5 h-5 text-slate-400 shrink-0 transition-transform duration-350 ${isOpen ? 'rotate-180 text-emerald-600 animate-pulse' : ''}`} />
                    </button>

                    <div 
                      className={`transition-all duration-350 ease-in-out overflow-hidden ${
                        isOpen ? 'max-h-[800px] border-t border-slate-100 dark:border-slate-800' : 'max-h-0'
                      }`}
                    >
                      <div className="p-5 text-slate-600 dark:text-slate-300 text-xs md:text-sm leading-relaxed font-semibold bg-slate-50/30 dark:bg-slate-900/40">
                        {item.answer}
                      </div>
                    </div>
                  </div>
      ...
      {selectedArticle && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-2xl w-full max-h-[85vh] overflow-y-auto p-6 md:p-10 shadow-2xl space-y-6 relative border border-slate-100 dark:border-slate-800 text-slate-800 dark:text-slate-100">
            <button 
              onClick={() => setSelectedArticle(null)}
              className="absolute top-4 right-4 bg-slate-100 dark:bg-slate-850 hover:bg-slate-200 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-300 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm cursor-pointer"
            >
              ✕
            </button>
            
            <div className="space-y-3">
              <span className="inline-block px-3 py-1 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 text-[10px] font-black uppercase tracking-wider rounded-full">
                {selectedArticle.category}
              </span>
              <h3 className="text-xl md:text-2xl font-black font-display text-slate-950 dark:text-slate-100 leading-tight">
                {selectedArticle.title}
              </h3>
              <p className="text-slate-400 dark:text-slate-450 text-[10px] font-bold flex items-center gap-1 pb-4 border-b border-slate-100 dark:border-slate-800">
                <Clock className="w-3.5 h-3.5 text-slate-350" />
                Temps de lecture : {selectedArticle.readTime}
              </p>
            </div>

            <div className="space-y-4 text-slate-700 dark:text-slate-300 text-xs sm:text-sm leading-relaxed font-semibold">
            ...
            <div className="pt-6 border-t border-slate-100 dark:border-slate-800 flex justify-end">
              <button 
                onClick={() => setSelectedArticle(null)}
                className="px-6 py-2.5 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white font-bold text-xs rounded-xl uppercase tracking-wider cursor-pointer"
              >
                Fermer
              </button>
            </div>
